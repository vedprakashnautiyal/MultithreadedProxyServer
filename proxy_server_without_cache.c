...
{ // Creating Socket for remote server
    ...
    // Get Host By Name Or IP Address Provided
...
cache_element *find(char *url)
{
    ...
    int temp_lock_val = pthread_mutex_lock(&lock); // Lock Acquires
    ...
    if (head != NULL)
    {
        ...
        while (site != NULL)
        {
            ...
            {
                printf("LRU Time Before : %ld\n", site->lru_time_track);
                printf("Url Found !!\n");
                // Updating the time_track of the most recenly used
                site->lru_time_track = time(NULL);
                printf("LRU Time After : %ld\n", site->lru_time_track);
                break;
            }
            site = site->next;
        }
    ...
    temp_lock_val = pthread_mutex_unlock(&lock); // Lock Release
...
void remove_cache_element()
{
    ...
    int temp_lock_val = pthread_mutex_lock(&lock); // Lock Acquired
    ...
    { // Cache != empty
        ...
        if (temp == head)
        {
            head = head->next; // Base Condition (Removing LRU Element)
        }
        else
        {
            p->next = temp->next;
        }
        cache_size = cache_size - (temp->len) - sizeof(cache_element) - strlen(temp->url) - 1; // updating the cache size after removing LRU element
        free(temp->data);
        free(temp->url);
        free(temp); // Free the removed element pointer
    }
    temp_lock_val = pthread_mutex_unlock(&lock); // Removing Lock
    printf("Removed Cache Element, Lock Unlocked %d\n", temp_lock_val);
}
...
int add_cache_element(char *data, int size, char *url)
{
    ...
    if (element_size <= MAX_ELEMENT_SIZE)
    {
        ...
        element->len = size;
        head = element;
        cache_size += element_size; // Incrementing Cache Size
        ret = 1;
    }
    temp_lock_val = pthread_mutex_unlock(&lock);
    printf("Added Cache Element, Lock Unlocked %d\n", temp_lock_val);
    free(url);
    free(data);
    printf("\ncache size:%d\n", cache_size);
    return ret;
